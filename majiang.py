...
class player:
	...
	def cant_see_left(self,pai):             #每张牌剩余的量
		...
		num = 0
		for i in range(0,len(self.user_desk.list_player)):
			num = num + self.user_desk.list_player[i].have_peng.count(pai) * 2
	...
	"""def cant_see_left(self,pai):
		...
		return self.user_desk.list_tuple.count(pai)"""

	def list_cant_see_left(self,list_x):
		num = 0
		for i in range(0, len(list_x)):
			num = num + self.cant_see_left(list_x[i])
		return num
		...
		if self.ding == 't':
			for i in range(0, len(self.list_w)):
				...
				if len(self.list_hope_pai) == 0:
					continue
				left_pai_num = self.list_cant_see_left(self.list_hope_pai)

				print("da:",tmp_pai,"max_score:",max_score,"list_hope_pai",self.list_hope_pai, " cant_see_left: ", left_pai_num)
				if left_pai_num > max_score_pai_num:
					max_score_pai_num = left_pai_num
