...
for x in range(0, 1000):
	...
	while len(desk1.list_tuple)>0 and len(list_player) > 0:
		...
		if list_player[index].score >= 14:
			send_data={'type':'zimo','player':list_player[index].name,'pai':list_player[index].mo}
			socket_test.send_msg(conn, bytes(json.dumps(send_data), encoding="utf-8"))
			...
			send_data={'type':'mo','player':list_player[index].name,'pai':list_player[index].mo}
			socket_test.send_msg(conn, bytes(json.dumps(send_data), encoding="utf-8"))
			str = input("\n")
			go_out = 0
			while 1 == 1:
				...
				list_player[index].show()
				send_data={'type':'da','player':list_player[index].name,'pai':list_player[index].da}
				socket_test.send_msg(conn, bytes(json.dumps(send_data), encoding="utf-8"))
				desk1.show()
				if len(list_player) > 1:
					for i in range(0,count_player):
						...
						if list_player[i].score >= 14:
							...
							list_player[i].show()
							send_data={'type':'dianpao','player':list_player[i].name,
											"last_player":list_player[index].name,'pai':pai}
							socket_test.send_msg(conn, bytes(json.dumps(send_data), encoding="utf-8"))
							list_player.pop(i)
							count_player = count_player -1
							index = i - 1
				...
				if len(list_player) > 1:
					for i in range(0, count_player):
						...
						if pai == list_player[i].peng(pai):
							...
							list_player[i].show()
							send_data={'type':'peng','player':list_player[i].name,
											"last_player":list_player[index].name,'pai':pai}
							socket_test.send_msg(conn, bytes(json.dumps(send_data), encoding="utf-8"))
							index = i	
							go_out = 0
