	...
	def peng(self,pai):
		...
		if self.nenpeng.count(pai) > 0:
			...
			if len(self.nenpeng) > 0:	
				...
				best_pai,max_score,max_score_pai_num = self.think(1)
				if max_score >= 14 and self.hu_pai_left_num > hu_pai_left_num + 1.5:
					return pai
				else:
					self.nenpeng.append(pai)
					self.have_peng.remove(pai)
			...
			for i in range(0, len(self.list_w)):
				...
				for j in range(11, 30):
					...
					if level > 1:
						...
						if tmp_max_score >= 14 and max_score_pai_num >= 1.4:
...
def test_spec():
	p1 = player();
	p1.name = 'pa'
	p1.list_t=[5,6,7,7,8,9]
	p1.list_w=[3,4,5,6,9]
	p1.list_ding = ['b','b','t','t']
	p1.have_peng.append(4)
	p1.user_desk.list_river.append(13)
	p1.user_desk.list_river.append(13)
	p1.user_desk.list_river.append(13)
	p1.user_desk.list_river.append(16)
	for i in range(0,50):
