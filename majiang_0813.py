...
class player:
	...
	score = 0; ding = 't'; value = 100; mo = 0; da = 0;user_desk = desk()
	dui = 0; name = "";hu_pai_left_num = 0;
	def __init__(self):
...
#					continue
				if level > 1:
					...
					if len(self.list_hope_pai) > max_score_pai_num:
						...
						if max_score >= 14:
							self.hu_pai_left_num = self.list_cant_see_left(self.list_hope_pai)
					elif len(self.list_hope_pai) == max_score_pai_num and max_score_pai_num !=0:
						if max_score >= 14 and self.list_cant_see_left(self.list_hope_pai) > self.hu_pai_left_num:
							self.hu_pai_left_num = self.list_cant_see_left(self.list_hope_pai)
							the_best_pai = tmp_pai
						elif self.pai_value(tmp_pai) < self.pai_value(the_best_pai):
							the_best_pai = tmp_pai
...
#			nodui_score = self.group_shunzi(self.list_t)*3 + self.group_shunzi(self.list_w)*3
	...
	def cant_see_left(self,pai):
		...
		for i in range(0,len(self.user_desk.list_player)):
			num = num + self.user_desk.list_player[i].have_peng.count(pai) * 2 + self.user_desk.list_player[i].have_gang.count(pai) * 3
	...
	def list_cant_see_left(self,list_x):
		...
		for i in range(0, len(list_x)):
			...
			if self.list_ding.count(self.get_pai_attr(list_x[i])) >= 2:
				tmp_num = tmp_num * 2
			num = num + tmp_num
		return num
